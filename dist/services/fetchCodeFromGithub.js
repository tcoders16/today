...
async function fetchCodeFromGitHub({ owner, repo, path, branch = "main" }) {
...
async function analyzeCodeWithAI(code, question) {
    const systemPrompt = `
...
export async function handleCodeAnalysis({ owner, repo, path, branch = "main", question }) {
    try {
        const code = await fetchCodeFromGitHub({ owner, repo, path, branch });
        const answer = await analyzeCodeWithAI(code, question);
        console.log(boxen(chalk.green(`File: ${path} on branch ${branch}`) +
            "\n\n" +
            chalk.white(answer), {
            padding: 1,
            borderStyle: "round",
