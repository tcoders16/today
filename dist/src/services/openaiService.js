...
`.trim();
    try {
        console.log(boxen(chalk.blueBright("💡 Sending your natural command to GPT-4o..."), { padding: 1, borderColor: "cyan", borderStyle: "round" }));
        ...
        console.log(boxen(chalk.greenBright("OpenAI Response Received:") +
            "\n\n" +
            chalk.white(content), { padding: 1, borderColor: "green", borderStyle: "round" }));
        const parsed = JSON.parse(content);
        console.log(boxen(chalk.cyanBright("🎯 Parsed Action Object:") +
            "\n\n" +
            chalk.white(JSON.stringify(parsed, null, 2)), { padding: 1, borderColor: "magenta", borderStyle: "round" }));
        return parsed;
    }
    catch (error) {
        ...
        if (typeof rawContent === "string" && rawContent.trim().startsWith("I") || (typeof rawContent === "string" && rawContent.includes("can"))) {
            console.log(boxen(chalk.yellowBright("🤖 Assistant Response:\n\n") + chalk.white(rawContent), { padding: 1, borderColor: "yellow", borderStyle: "round" }));
        }
        else {
            console.log(boxen(chalk.redBright(" Failed to parse command:\n\n") + chalk.white(error.message), { padding: 1, borderColor: "red", borderStyle: "round" }));
        }
        return {};
    }
}
