...
type AnalyzeFileParams = {
  ...
  functionName?: string;    // required only if mode === "function"
...
export async function analyzeFile({
  ...
  branch = "main",
  mode
}: AnalyzeFileParams): Promise<void> {
  try {
    ...
    if (mode === "function" && functionName) {
      const regex = new RegExp(
        `(function\\s+${functionName}\\s*\$begin:math:text$[^)]*\\$end:math:text$\\s*{[\\s\\S]*?})|(const\\s+${functionName}\\s*=\\s*\$begin:math:text$[^)]*\\$end:math:text$\\s*=>\\s*{[\\s\\S]*?})`
      );
    ...
    console.error(
      chalk.red("Error analyzing file:"),
